public class Nash extends Personagem {

    public Nash(char PosicaoInicial){

                //Pontos Fixos
        setName("Nash");
        setPower(3);
        setHealth(3) ;
        setMobility(4);
        setTechniques(3);
        setRange(3);
        setDescription("Paradoxiacal Avenger");

                //Pontos Variados
        setLife(1000*(getHealth())); 
        setDamage(); 
        setX(PosicaoInicial);
        setDefesa(false); 
		setSpeed(); 
 
    }
    public Object[] AtaquesEspeciais(int Especial){

        Object[] Lista_Parametros = new Object[4];

        Lista_Parametros[0] = (int)getPosicao()[0];
        Lista_Parametros[1] = (int)getPosicao()[1];

        int Range;
        double Damage;

        switch (Especial){
            case  1 :
                System.out.println(getName()+"  Dragon Suplex ");
                Lista_Parametros[2] = (getRange()* 18 );
                Lista_Parametros[3] = (getDamage()* 6 );
                break;
            case  2 :
                System.out.println(getName()+"  Target Down ");
                Lista_Parametros[2] = (getRange()* 7 );
                Lista_Parametros[3] = (getDamage()* 3 );
                break;
            case  3 :
                System.out.println(getName()+"  Air Jack ");
                Lista_Parametros[2] = (getRange()* 5 );
                Lista_Parametros[3] = (getDamage()* 4 );
                break;
            case  4 :
                System.out.println(getName()+"  Bullet Clear ");
                Lista_Parametros[2] = (getRange()* 21 );
                Lista_Parametros[3] = (getDamage()* 5 );
                break;
            case  5 :
                System.out.println(getName()+"  Sonic Move - Hide  ");
                Lista_Parametros[2] = (getRange()* 27 );
                Lista_Parametros[3] = (getDamage()* 11 );
                break;
            case  6 :
                System.out.println(getName()+"  Sonic Move - Blitz Air  ");
                Lista_Parametros[2] = (getRange()* 11 );
                Lista_Parametros[3] = (getDamage()* 2 );
                break;
            case  7 :
                System.out.println(getName()+"  Sonic Move - Steel Air  ");
                Lista_Parametros[2] = (getRange()* 17 );
                Lista_Parametros[3] = (getDamage()* 6 );
                break;
            case  8 :
                System.out.println(getName()+"  Stealth Dash  ");
                Lista_Parametros[2] = (getRange()* 16 );
                Lista_Parametros[3] = (getDamage()* 2 );
                break;
            case  9 :
                System.out.println(getName()+"  Sonic Move - Avoid  ");
                Lista_Parametros[2] = (getRange()* 15 );
                Lista_Parametros[3] = (getDamage()* 2 );
                break;
            case  10 :
                System.out.println(getName()+"  Knee Bazooka ");
                Lista_Parametros[2] = (getRange()* 8 );
                Lista_Parametros[3] = (getDamage()* 3 );
                break;
            case  11 :
                System.out.println(getName()+"  Jumping Sobat ");
                Lista_Parametros[2] = (getRange()* 13 );
                Lista_Parametros[3] = (getDamage()* 6 );
                break;
            case  12 :
                System.out.println(getName()+"  Chopping Assault ");
                Lista_Parametros[2] = (getRange()* 23 );
                Lista_Parametros[3] = (getDamage()* 8 );
                break;
            case  13 :
                System.out.println(getName()+"  Step Kick ");
                Lista_Parametros[2] = (getRange()* 23 );
                Lista_Parametros[3] = (getDamage()* 6 );
                break;
            case  14 :
                System.out.println(getName()+"  Spinning Back Knuckle ");
                Lista_Parametros[2] = (getRange()* 24 );
                Lista_Parametros[3] = (getDamage()* 7 );
                break;
            case  15 :
                System.out.println(getName()+"  Side Knee Attack ");
                Lista_Parametros[2] = (getRange()* 23 );
                Lista_Parametros[3] = (getDamage()* 2 );
                break;
            case  16 :
                System.out.println(getName()+"  Rapid Punch ");
                Lista_Parametros[2] = (getRange()* 5 );
                Lista_Parametros[3] = (getDamage()* 3 );
                break;
            case  17 :
                System.out.println(getName()+"  Rapid Kick ");
                Lista_Parametros[2] = (getRange()* 23 );
                Lista_Parametros[3] = (getDamage()* 3 );
                break;
            case  18 :
                System.out.println(getName()+"  Wind Shear ");
                Lista_Parametros[2] = (getRange()* 10 );
                Lista_Parametros[3] = (getDamage()* 3 );
                break;
            case  19 :
                System.out.println(getName()+"  Down Burst ");
                Lista_Parametros[2] = (getRange()* 27 );
                Lista_Parametros[3] = (getDamage()* 11 );
                break;
            case  20 :
                System.out.println(getName()+"  Raptor Combination ");
                Lista_Parametros[2] = (getRange()* 25 );
                Lista_Parametros[3] = (getDamage()* 10 );
                break;
            case  21 :
                System.out.println(getName()+"  Bullet Combination ");
                Lista_Parametros[2] = (getRange()* 17 );
                Lista_Parametros[3] = (getDamage()* 10 );
                break;
            case  22 :
                System.out.println(getName()+"  Sonic Boom  ");
                Lista_Parametros[2] = (getRange()* 10 );
                Lista_Parametros[3] = (getDamage()* 7 );
                break;
            case  23 :
                System.out.println(getName()+"  Sonic Scythe  ");
                Lista_Parametros[2] = (getRange()* 19 );
                Lista_Parametros[3] = (getDamage()* 6 );
                break;
            case  24 :
                System.out.println(getName()+"  Moonsault Slash  ");
                Lista_Parametros[2] = (getRange()* 26 );
                Lista_Parametros[3] = (getDamage()* 10 );
                break;
            case  25 :
                System.out.println(getName()+"  Tragedy Assault  ");
                Lista_Parametros[2] = (getRange()* 27 );
                Lista_Parametros[3] = (getDamage()* 7 );
                break;
            case  26 :
                System.out.println(getName()+"  Stealth Dash ");
                Lista_Parametros[2] = (getRange()* 23 );
                Lista_Parametros[3] = (getDamage()* 10 );
                break;
            case  27 :
                System.out.println(getName()+"  Stealth Dash (Stop) ");
                Lista_Parametros[2] = (getRange()* 27 );
                Lista_Parametros[3] = (getDamage()* 6 );
                break;
            case  28 :
                System.out.println(getName()+"  Justice Corridor ");
                Lista_Parametros[2] = (getRange()* 24 );
                Lista_Parametros[3] = (getDamage()* 7 );
                break;
            case  29 :
                System.out.println(getName()+"  Justice Shell ");
                Lista_Parametros[2] = (getRange()* 14 );
                Lista_Parametros[3] = (getDamage()* 4 );
                break;
            case  30 :
                System.out.println(getName()+"  Judgement Saber ");
                Lista_Parametros[2] = (getRange()* 17 );
                Lista_Parametros[3] = (getDamage()* 2 );
                break;
            default:
                System.out.println("Habilidade Inexistente");
                Lista_Parametros[2] = getRange();
                Lista_Parametros[3] = getDamage();
                break;
        }


        return Lista_Parametros;
    }
}
